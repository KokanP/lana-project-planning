...
WHALES_ANALYSIS_URL = "whales-analysis.html"
...
def plot_top_n_chart(holders_data, num_holders_to_plot, filename):
    ...
    print(f"\nGenerating Top {num_holders_to_plot} Holders Bar Chart...", file=sys.stderr)
    if not holders_data or len(holders_data) < num_holders_to_plot:
        print(f"Error: Not enough holder data ({len(holders_data)}) to plot top {num_holders_to_plot}.", file=sys.stderr)
        return False
    try:
        ...
        plt.close()
        print(f"Successfully saved top holders chart to {filename}", file=sys.stderr)
        return True
...
def plot_lorenz_curve(holders_data, filename):
    ...
    # Log progress to stderr
    print("\nGenerating Lorenz Curve for Wealth Distribution...", file=sys.stderr)
    if not holders_data:
        print("Error: No holder data available for Lorenz curve.", file=sys.stderr)
        return False, None
    try:
        # ... (plotting logic remains the same) ...
        ...
        if len(balances_lorenz_coins) == 0:
            print("Error: No valid positive balances found for Lorenz curve.", file=sys.stderr)
            return False, None
        total_balance_coins = balances_lorenz_coins.sum()
        if total_balance_coins == 0:
             print("Error: Total balance is zero, cannot generate Lorenz curve.", file=sys.stderr)
             return False, None
        ...
        plt.savefig(filename)
        plt.close()
        print(f"Successfully saved Lorenz curve to {filename}", file=sys.stderr)
        return True, gini
    except Exception as e:
...
def plot_balance_histogram(holders_data, filename):
    ...
    # Log progress to stderr
    print("\nGenerating Balance Distribution Histogram...", file=sys.stderr)
    if not holders_data:
        print("Error: No holder data available for histogram.", file=sys.stderr)
        return False
    try:
        # ... (plotting logic remains the same) ...
        ...
        if len(balances_hist) == 0:
            print("Error: No valid positive balances found for histogram.", file=sys.stderr)
            return False
        ...
        plt.savefig(filename)
        plt.close()
        print(f"Successfully saved balance histogram to {filename}", file=sys.stderr)
        return True
    except Exception as e:
...
def calculate_and_format_concentration(holders_data, circulating_supply):
    ...
    # Log progress to stderr
    print("\nCalculating concentration...", file=sys.stderr)
    conc_10_str = "N/A"
    conc_100_str = "N/A"

    if not holders_data:
        print("Cannot calculate concentration: No parsed holder data.", file=sys.stderr)
    ...
    if circulating_supply is None or not isinstance(circulating_supply, (int, float)) or circulating_supply <= 0:
        print(f"Cannot calculate concentration: Invalid circulating supply ({circulating_supply}).", file=sys.stderr)
        return conc_10_str, conc_100_str

    filtered_holders = [h for h in holders_data if isinstance(h.get('balance'), (int, float))]
    if not filtered_holders:
        print("Cannot calculate concentration: No holders with valid balances found.", file=sys.stderr)
        return conc_10_str, conc_100_str
    ...
    try:
        ...
        print("Calculations complete.", file=sys.stderr)
    except Exception as e:
...
def run_analysis():
    ...
    # Log progress to stderr
    print("Starting concentration analysis with HTML plotting...", file=sys.stderr)
    ...
    print("Fetching circulating supply...", file=sys.stderr)
    ...
    if circulating_supply_data is not None:
        try:
            ...
            print(f"Received circulating supply: {circulating_supply}", file=sys.stderr)
        except ValueError:
    ...
    print(f"Waiting {API_DELAY}s...", file=sys.stderr)
    time.sleep(API_DELAY)
    print("Fetching rich list (top 1000)...", file=sys.stderr)
    ...
    if rich_list_data and isinstance(rich_list_data, dict):
        ...
        if isinstance(holders_list, list):
            print("\nParsing rich list data...", file=sys.stderr)
            temp_holders = []
            ...
            print(f"Successfully parsed {len(parsed_holders)} entries from rich list.", file=sys.stderr)
    ...
    print("\nEncoding images for HTML embedding...", file=sys.stderr)
    ...
    print("\nFormatting HTML report...", file=sys.stderr)
    ...
    # Basic CSS for styling (Removed yellow background test)
    html_style = """
<style>
...
"""
    ...
    # This is the *only* print to stdout needed for the workflow redirection
    print(html_string)
    ...
    for plot_file in [PLOT_TOP_N_FILENAME, PLOT_LORENZ_FILENAME, PLOT_HIST_FILENAME]:
        if os.path.exists(plot_file):
            try:
                ...
                print(f"Removed temporary plot file: {plot_file}", file=sys.stderr)
            except OSError as e:
